...
ld=args[1]
tag_rsid_path=args[2]
tag2corr_r2_tsv=args[3]
}
...
#setkey(indexld, snpb)
#setkey(tagld, snpa)
#tag2tagcorr_r2 = unique(merge(indexld, tagld, all=FALSE, by.x=c('snpb'), by.y=c('snpa'), allow.cartesian=TRUE))
...
write.table(tagld, tag2corr_r2_tsv, quote=FALSE, sep="\t", row.names=FALSE)
